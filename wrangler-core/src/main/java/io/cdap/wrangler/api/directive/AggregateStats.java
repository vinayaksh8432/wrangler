...
public class AggregateStats implements Directive {
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context) {
            ...
            if (sizeObj != null && !sizeObj.toString().isEmpty()) {
                try {
                    ...
                    if (sizeStr.endsWith("MB")) {
                        String numberStr = sizeStr.substring(0, sizeStr.length() - 2);
                        totalSize += Long.parseLong(numberStr);
                    } else {
                        ByteSize size = new ByteSize(sizeStr);
                        totalSize += size.getBytes() / (1024 * 1024); // Convert bytes to MB
                    }
                } catch (Exception e) {
        ...
        String totalSizeStr = String.format("%dMB", totalSize);
        result.add(targetSizeColumn.value(), totalSizeStr);
